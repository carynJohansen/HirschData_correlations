...
exp.long <- melt(maize_exp, id=c("gene", "chromosome", "position_left", "position_right"))
...
small <- traitData[1:10,1:20]

small.cor <- cor(t(small), method="pearson", use="complete.obs")
heatmap.2(small.cor, trace="none", Colv = "none", Rowv=F, dendrogram = "none")

small.cor.phy <- small.cor[1:17, 1:17]
heatmap.2(small.cor.phy, trace="none")
sml.cor.l <- unlist(small.cor)
hist(sml.cor.l)
#z-score 
...
#ggplot(trait.phys.m, aes(x=log(value), y=..density..)) +
#  geom_histogram() +
#  facet_wrap(~ trait, scale="free")
...
pc1.2 <- qplot(z=PC1, y=PC2, data=trait.pca.x, colour=factor(gty))

phys.cor <- cor(t(trait.phys), method="pearson", use="complete.obs")

heatmap.2(phys.cor, trace="none")



#exp_cor <- cor(exp, method="pearson", use="complete")
```
